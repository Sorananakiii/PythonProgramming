...
When choosing a collection type, it is useful to understand the unique properties of that type. For example, a list is for us to store multiple objects and to retain a sequence, a dictionary is for us to store unique key-value pair mappings, tuples are immutable, and sets only store unique elements. Choosing the right type for a particular dataset could mean an increase in efficiency or security.
